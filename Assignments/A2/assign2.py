...
raw_df = pd.read_csv("3D_spatial_network.txt", sep=',', header=None,  #Reading the data
                     names=columns).drop("useless", 1).sample(frac=1)

deg = input("Enter the Degree")  
pre_processor = Setup(deg=int(deg))
X_train, Y_train, x_val, y_val, x_test, y_test = pre_processor.train_test_split(raw_df) #Splitting the data
